...
4. 표시된 **Clerk domain**을 복사 (예: `your-app-12.clerk.accounts.dev`)
   - 이 domain은 다음 단계에서 사용합니다

**3-2. Supabase Dashboard에서 Clerk Third-Party Auth 설정**
...
5. 3-1에서 복사한 **Clerk domain**을 입력 (예: `your-app-12.clerk.accounts.dev`)
6. **"Save"** 클릭

**3-3. 로컬 개발 환경 설정 (선택사항)**

로컬에서 Supabase CLI를 사용하는 경우, `supabase/config.toml` 파일에 다음 설정을 추가:

```toml
[auth.third_party.clerk]
enabled = true
domain = "your-app-12.clerk.accounts.dev"  # 3-1에서 복사한 Clerk domain
```

**3-4. 통합 확인**

- [Clerk 공식 통합 가이드](https://clerk.com/docs/guides/development/integrations/databases/supabase)에서 상세 정보 확인
- [Supabase Third-Party Auth 문서](https://supabase.com/docs/guides/auth/third-party/clerk) 참고
...
1. [Supabase Dashboard](https://supabase.com/dashboard) → 프로젝트 선택 → **Settings** → **API**
2. 다음 값들을 복사하여 `.env` 파일에 입력:
   ...
   # ⚠️ 주의: 서버 사이드에서만 사용, 클라이언트에 노출 금지!
...
프로젝트에 Clerk 한국어 로컬라이제이션이 적용되어 있습니다.

### 설정 위치

- **`app/layout.tsx`**: `ClerkProvider`에서 `koKR` locale 설정
- **`@clerk/localizations`**: 한국어 번역 패키지 사용

### 적용 범위

- ✅ 모든 Clerk 컴포넌트 (SignIn, SignUp, UserButton 등)
- ✅ 에러 메시지
- ✅ 폼 레이블 및 버튼 텍스트
- ❌ Clerk Account Portal (호스팅된 포털은 영어로 유지)

### 커스텀 에러 메시지

`app/layout.tsx`에서 커스텀 에러 메시지를 추가할 수 있습니다:
...
│   │   ├── server.ts          # Server Component용 (Cookie 기반, 공식 문서 방식)
│   │   ├── client.ts          # Client Component용 (Browser 클라이언트)
│   │   ├── service-role.ts    # 관리자용 (RLS 우회)
│   │   └── middleware.ts      # Middleware용 헬퍼 (참고용)
